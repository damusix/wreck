...
Parses the provided _cache-control_ request header value into an object containing
...
The request event is emitted just before _wreck_ creates a request. The
...
The request event is emitted just after _wreck_ creates a request. The handler should accept the following arguments `(req)` where:
...
The response event is always emitted for any request that _wreck_ makes. The
...
be aware that if multiple modules are depending on the same cached _wreck_
...
useful for determining how long it takes _wreck_ to get a response back and
processed.
